...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $products = mysqli_query($connect, "SELECT * FROM `product` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $products = mysqli_query($connect, "SELECT * FROM `product` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
            ...
            <div class="product-search">
                <form id="product-search-form" action="product_listing.php?action=search" method="POST">
                    <fieldset>
                        ...
                        Tên sản phẩm: <input type="text" name="TENSANPHAM" value="<?=!empty($name)?$name:""?>" />
...
<div class="clear-both"></div>
                ...
                while ($row = mysqli_fetch_array($products)) {
                    ?>
                    <li>
                        <div class="product-prop product-img"><img src="./uploads/<?= $row['image'] ?>" alt="<?= $row['name'] ?>" title="<?= $row['name'] ?>" /></div>
